...
_SCHEMA = {
    "type": "object",
    "properties": {
        "wind_speed_mph": {"type": "number", "description": "current sustained wind speed in mph"},
        "wind_gust_mph": {"type": "number", "description": "current wind gust speed in mph"},
    ...
    "required": ["wind_speed_mph", "temp_c"],
}
...
async def fetch_weather(location: str) -> WeatherReading:
    ...
    if data.get("wind_speed_mph") is None:
    ...
    return WeatherReading(
        ...
        wind_gust_mph=float(data["wind_gust_mph"]) if data.get("wind_gust_mph") else None,
